    ...
    for (let i = 1; i <= loanDetail?.loanDurationInMonth; i++) {
      ...
      content.push(
        <TableRow key={item.i}>
          ...
          <TableCell align='center' component='th' scope='row' sx={{ border: '1px solid rgba(224, 224, 224, 1)' }}>
            {moment(startLoanDate).add(i, 'months').format('MM') +
              '/' +
              moment(startLoanDate).add(543, 'years').format('YYYY')}
          </TableCell>
    ...
    <Grid container spacing={2} sx={{ pl: 15, pr: 15 }}>
      ...
      <Grid item xs={5} sx={{ display: 'flex', justifyContent: 'right' }}>
        <Grid item xs={12}>
          ...
          <Typography variant='body2'>
            วันที่ {moment().format('DD')} เดือน {moment().format('MMMM')} พ.ศ.{' '}
      ...
      <Grid item xs={12} sm={12} sx={{ display: 'flex', justifyContent: 'left' }}>
        <Typography variant='body2' sx={{ lineHeight: 2, justifyContent: 'justify' }} paragraph='true'>
          ข้าพเจ้า (นาย, นาง, นางสาว) {loanDetail?.memberName} ที่อยู่ {loanDetail?.houseNo} ถ.
          {loanDetail?.streetName ?? '-'} ม.{loanDetail?.villageNo} ต.{loanDetail?.subDistrict} อ.{loanDetail?.district}{' '}
          จ.{loanDetail?.province} รหัสไปรษณีย์ {loanDetail?.postCode}
          &nbsp;ได้รับเงินสวัสดิการ{loanDetail?.loanMainTypeId == 1 ? 'ฉุกเฉิน' : 'สามัญ'} กลุ่มสวัสดิการครูดารุสสาลาม
          จำนวน {loanDetail?.loanAmount} บาท ({THBText(loanDetail?.loanAmount)}) โดยกำหนดชำระดังต่อไปนี้
        </Typography>
      </Grid>
        ...
        <TableContainer sx={{ ml: 12 }}>
          ...
          >
            <TableHead>
              <TableRow>
                <TableCell align='center' sx={{ border: '1px solid rgba(224, 224, 224, 1)' }}>
                  <Typography variant='body2'>งวดที่</Typography>
                </TableCell>
                <TableCell align='center' sx={{ border: '1px solid rgba(224, 224, 224, 1)' }}>
                  <Typography variant='body2'>วันที่</Typography>
                </TableCell>
                <TableCell align='center' sx={{ border: '1px solid rgba(224, 224, 224, 1)' }}>
                  <Typography variant='body2'>เงินต้นที่จ่ายต่อเดือน</Typography>
                </TableCell>
                <TableCell align='center' sx={{ border: '1px solid rgba(224, 224, 224, 1)' }}>
                  <Typography variant='body2'>กำไรต่เดือน</Typography>
                </TableCell>
                <TableCell align='center' sx={{ border: '1px solid rgba(224, 224, 224, 1)' }}>
                  <Typography variant='body2'>เงินที่ต้องจ่ายต่อเดือน</Typography>
                </TableCell>
                <TableCell align='center' sx={{ border: '1px solid rgba(224, 224, 224, 1)' }}>
            ...
            <TableBody>{getPlansContent()}</TableBody>
          </Table>
        </TableContainer>
      </Grid>
      <Grid item xs={12}>
        <Typography variant='body2'>
          ข้าพเจ้ายินยอมให้ทางโรงเรียนหักเงินเดือนของข้าพเจ้าตามตารางที่กำหนดข้างต้น
        </Typography>
      </Grid>
      {loanDetail?.loanTypeId > 2 ? (
        <Grid item xs={6} sx={{ display: 'flex', justifyContent: 'center', mt: 5 }}>
          <Grid item xs={12}>
            ...
            <Typography variant='body2'>
              ลงวันที่ {moment(loanDetail?.loanApprovedAt).add(543, 'year').format('DD/MM/YYYY')}
            </Typography>
            <Typography variant='body2'>ลงชื่อ......................................คู่สมรส</Typography>
            <Typography variant='body2' sx={{ display: 'flex', pl: 10 }}>
              ( {loanDetail?.spouseName} )
            </Typography>
          </Grid>
        </Grid>
      ) : (
