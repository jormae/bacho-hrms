    ...
    <Grid container spacing={2} sx={{ pl: 15, pr: 15 }}>
      ...
      <Grid item xs={12} sm={12} sx={{ display: 'flex', justifyContent: 'left' }}>
        <Typography variant='body2' sx={{ lineHeight: 2, justifyContent: 'justify' }} paragraph='true'>
          คำให้การค้ำประกัน ข้าพเจ้า (นาย, นาง, นางสาว) {loanDetail?.secondReferenceName} เลขที่บัตรประชาชน{' '}
          {loanDetail?.secondReferenceId} โทรศัพท์ {loanDetail?.secondReferenceContactNo}
          &nbsp; ขอค้ำประกันเงินสวัสดิการครูดารุสสาลามของ (นาย, นาง, นางสาว) {
            loanDetail?.memberName
          } เลขที่บัตรประชาชน {loanDetail?.nationalId}
          &nbsp; ตามหนังสือสัญญาเลขที่ {loanDetail?.refId} ลงวันที่ {moment(loanDetail?.loanApprovedAt).format('DD')}{' '}
          เดือน {moment(loanDetail?.loanApprovedAt).format('MMMM')} พ.ศ.{' '}
          {moment(loanDetail?.loanApprovedAt).add(543, 'year').format('YYYY')}{' '}
          ถ้าหากกองทุนสวัสดิการไม่สามารถเรียกเก็บเงินจาก (นาย, นาง, นางสาว) {loanDetail?.memberName}{' '}
          ข้าพเจ้ายินยอมชดใช้กองทุนให้ครบถ้วน
        </Typography>
      </Grid>
      <Grid item xs={6} />
      ...
      <Grid item xs={12} sm={12} sx={{ display: 'flex', justifyContent: 'left' }}>
        <Typography variant='body2' sx={{ lineHeight: 2, justifyContent: 'justify' }} paragraph='true'>
          ข้าพเจ้า (นาย, นาง, นางสาว) {loanDetail?.secondSpouseName} เลขที่บัตรประชาชน{' '}
          {loanDetail?.secondSpouseNationalId} โทรศัพท์ {loanDetail?.secondSpouseContactNo}
          &nbsp; คู่สมรสของ {loanDetail?.secondReferenceName} ยินยอมให้ (นาย, นาง, นางสาว){' '}
          {loanDetail?.secondReferenceName}
          &nbsp; ค้ำประกันเงินสวัสดิการสามัญ ครูดารุสสาลาม ตามหนังสือสัญญาเลขที่ {loanDetail?.refId} ลงวันที่{' '}
          {moment(loanDetail?.loanApprovedAt).format('DD')} เดือน {moment(loanDetail?.loanApprovedAt).format('MMMM')}{' '}
          พ.ศ. {moment(loanDetail?.loanApprovedAt).add(543, 'year').format('YYYY')}
        </Typography>
      </Grid>
      <Grid item xs={6} />
