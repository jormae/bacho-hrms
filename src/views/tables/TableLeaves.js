...
const TableLeaves = () => {
  ...
  const fetchLeaves = async () => {
    ...
    console.log('fetchLeaves uri = ' + uri)
    try {
  ...
  return (
    <Card>
      <CardHeader
        title={`รายงานสรุปข้อมูลการลาปฎิบัติงานประจำปีงบประมาณ ${strYearBudget}`}
        sx={{ textAlign: 'center' }}
        titleTypographyProps={{ variant: 'h3' }}
      />
      <Typography sx={{ textAlign: 'center', marginBottom: 4 }}>{`${mainDeptName} ${
        selectedDeptName['blogs'] ?? ''
      }`}</Typography>
      <Divider sx={{ margin: 0 }} />
      <CardContent>
            ...
            <Grid container spacing={5}>
              ...
              <Grid item xs={2}>
                <FormControl fullWidth>
                  ...
                  <Select fullWidth label='ปีงบประมาณ' value={selectedYear} onChange={handleYearBudgetChange}>
                ...
                leaves.blogs
                  ...
                  .map(row => (
                    <TableRow key={row.staffName}>
                      ...
                      <TableCell align='center'>{row.vacationLeave ?? '-'}</TableCell>
                      <TableCell align='center'>{row.personalLeave ?? '-'}</TableCell>
                      <TableCell align='center'>{row.medicalLeave ?? '-'}</TableCell>
                      <TableCell align='center'>{row.birthLeave ?? '-'}</TableCell>
                      <TableCell align='center'>{row.wifeCareLeave ?? '-'}</TableCell>
                      <TableCell align='center'>{row.hajjLeave ?? '-'}</TableCell>
                      <TableCell align='center'>{row.officialLeave ?? '-'}</TableCell>
