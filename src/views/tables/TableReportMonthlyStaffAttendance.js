...
const TableReportMonthlyStaffAttendance = () => {
  ...
  const strMonth = ' เดือน ' + moment(date).format('MMMM') + ' พ.ศ.' + moment(date).add(543, 'year').format('YYYY')
    ...
    <Card>
      ...
      <CardContent>
        ...
        <Link passHref href={`../../reports/monthly/attendance/print/${cid}/${date}`} color='success'>
          <Button type='button' variant='outlined' startIcon={<PrintIcon />} sx={{ mr: 2 }}>
            พิมพ์รายงานลงเวลา
          </Button>
        </Link>
        <Link passHref href={`../../reports/monthly/leave/print/${cid}/${date}`} color='success'>
          <Button type='button' variant='outlined' startIcon={<PrintIcon />}>
            พิมพ์รายงานลา
          </Button>
        </Link>
      </CardContent>
