...
const TableLeavesHeadDeptSigned = () => {
    ...
    const handleChangeDept = async data => {
        let uri = apiConfig.baseURL + `/leaves/leave-status-dept/2/${yearBudget}/${data.target.value}`
        console.log(uri)
        try {
            const { data } = await axios.get(uri)
    ...
    const fetchLeaves = async () => {
        let uri = apiConfig.baseURL + `/leaves/leave-status/2/${yearBudget}`
        console.log(uri)
        try {
            const { data } = await axios.get(uri)
            setLeaves({ blogs: data })
                            ...
                            {leaves.blogs
                                ...
                                .map(row => (
                                    <TableRow key={row.leaveId}>
                                        ...
                                        <TableCell align='center'>
                                            {moment(row.headSignDateTime).add(543, 'year').format('DD/MM/YYYY')}
                                        </TableCell>
...
export default TableLeavesHeadDeptSigned
