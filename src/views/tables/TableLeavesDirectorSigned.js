...
const TableLeavesDirectorSigned = () => {
    ...
    const handleChangeDept = async data => {
        let uri = apiConfig.baseURL + `/leaves/leave-status-dept/5/${yearBudget}/${data.target.value}`
        console.log(uri)
        try {
            const { data } = await axios.get(uri)
    ...
    const fetchLeaves = async () => {
        let uri = apiConfig.baseURL + `/leaves/leave-status/5/${yearBudget}`
        console.log(uri)
        try {
            const { data } = await axios.get(uri)
            setLeaves({ blogs: data })
    ...
    return (
        <Card>
            <CardHeader title={`รายงานข้อมูลการลาปฎิบัติงานสถานะอนุมัติ ประจำปีงบประมาณ ${thYearBudget}`} sx={{ textAlign: "center" }} titleTypographyProps={{ variant: 'h3' }} />
            <Divider sx={{ margin: 0 }} />
            <CardContent>
                            ...
                            {leaves.blogs
                                ...
                                .map(row => (
                                    <TableRow key={row.leaveId}>
                                        ...
                                        <TableCell align='center'>
                                            {moment(row.directorSignDateTime).add(543, 'year').format('DD/MM/YYYY')}
                                        </TableCell>
...
export default TableLeavesDirectorSigned
