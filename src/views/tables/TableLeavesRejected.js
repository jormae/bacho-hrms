...
const TableLeavesRejected = () => {
    ...
    const handleChangeDept = async data => {
        let uri = apiConfig.baseURL + `/leaves/leave-status-dept/6/${yearBudget}/${data.target.value}`
        console.log(uri)
        try {
            const { data } = await axios.get(uri)
    ...
    const fetchLeaves = async () => {
        let uri = apiConfig.baseURL + `/leaves/leave-status/6/${yearBudget}`
        console.log(uri)
        try {
            const { data } = await axios.get(uri)
            setLeaves({ blogs: data })
    ...
    return (
        <Card>
            <CardHeader title={`รายงานข้อมูลการลาปฎิบัติงานสถานะไม่อนุมัติ ประจำปีงบประมาณ ${thYearBudget}`} sx={{ textAlign: "center" }} titleTypographyProps={{ variant: 'h3' }} />
            <Divider sx={{ margin: 0 }} />
            <CardContent>
...
export default TableLeavesRejected
