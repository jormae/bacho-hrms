...
const TableLeavesNew = () => {
    ...
    const handleChangeDept = async data => {
        let uri = apiConfig.baseURL + `/leaves/leave-status-new/${data.target.value}`
        console.log(uri)
        try {
            const { data } = await axios.get(uri)
    ...
    const fetchLeaves = async () => {
        let uri = apiConfig.baseURL + `/leaves/leave-status-new`
        console.log(uri)
        try {
            const { data } = await axios.get(uri)
            setLeaves({ blogs: data })
    ...
    return (
        <Card>
            <CardHeader title={`รายงานข้อมูลการลาปฎิบัติงานสถานะรออนุมัติ ประจำปีงบประมาณ ${thYearBudget}`} sx={{ textAlign: "center" }} titleTypographyProps={{ variant: 'h3' }} />
            <Divider sx={{ margin: 0 }} />
            <CardContent>
                    ...
                    <Table sx={{ minWidth: 650 }} aria-label='simple table'>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell align='center'>วันที่เขียนใบลา</TableCell>
                            ...
                            {leaves.blogs
                                ...
                                .map(row => (
                                    <TableRow key={row.leaveId}>
                                        ...
                                        <TableCell align='center'>
                                            {row.createdAt ? moment(row.createdAt).add(543, 'year').format('DD/MM/YYYY') : '-'}
                                        </TableCell>
                                        <TableCell align='center'>
                                            {row.colleagueSignDateTime ? moment(row.colleagueSignDateTime).add(543, 'year').format('DD/MM/YYYY') : '-'}
                                        </TableCell>
                                        <TableCell align='center'>
                                            {row.headSignDateTime ? moment(row.headSignDateTime).add(543, 'year').format('DD/MM/YYYY') : '-'}
                                        </TableCell>
                                        <TableCell align='center'>
